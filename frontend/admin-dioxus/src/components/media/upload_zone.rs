...
pub fn MediaUploadZone(props: MediaUploadZoneProps) -> Element {
    ...
    let open_picker = {
        let id = input_id();
        move |_| {
            tracing::debug!("[MediaUploadZone] Opening file picker");
    ...
    let handle_file_change = move |_evt: Event<FormData>| {
        tracing::debug!("[MediaUploadZone] File change event triggered");
                    ...
                    if let Some(el) = doc.get_element_by_id(&id) {
                        if let Ok(target) = el.dyn_into::<HtmlInputElement>() {
                            if let Some(files) = target.files() {
                                ...
                                tracing::debug!("[MediaUploadZone] Files selected: {}", file_count);
                                        ...
                                        for i in 0..limit {
                                            if let Some(file) = files.get(i as u32) {
                                                ...
                                                if validate_file_type(&file, &allowed_refs) {
                                                    ...
                                                    tracing::warn!("[MediaUploadZone] File type not allowed: {}", file.type_());
                                                }
                                            }
                                        }
                                ...
                                tracing::warn!("[MediaUploadZone] No files found in input");
                            }
                        }
                    }
                }
...
async fn process_files_async(
    ...
    tracing::debug!("[process_files_async] Starting file processing");
    ...
    tracing::debug!("[process_files_async] Processing limit: {} files", limit);
    ...
    for i in 0..limit {
        if let Some(file) = files.get(i as u32) {
            ...
            tracing::debug!("[process_files_async] File type validated, creating payload");
            ...
            tracing::warn!("[process_files_async] Could not get file at index: {}", i);
        }
    }
